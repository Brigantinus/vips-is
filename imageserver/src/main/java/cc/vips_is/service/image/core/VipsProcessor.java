...
public class VipsProcessor {
    ...
    public ImageData readImageData(String identifier, byte[] bytes) {
        log.debug("readImageData (bytes): identifier={}", identifier);

        final Integer[] dimensions = new Integer[4];
        try {
            Vips.run(arena -> {
                VImage image = VImage.newFromBytes(arena, bytes);
                dimensions[0] = image.getWidth();
                dimensions[1] = image.getHeight();
                try {
                    dimensions[2] = image.getInt("tile-width");
    ...
    public byte[] processToBytes(byte[] originalBytes, Region region, Size size,
                                  RotationInfo rotation, QualityMode quality, ImageFormat format) {
        log.debug("processToBytes: region={}, size={}, rotation={}, quality={}, format={}", region, size, rotation, quality, format);

        ByteArrayOutputStream baos = new ByteArrayOutputStream();
        try {
            Vips.run(arena -> {
                VImage image = VImage.newFromBytes(arena, originalBytes);
                image = applyCrop(image, region, image.getWidth(), image.getHeight());
                image = applyResize(image, size);
                image = applyRotation(image, rotation);
                image = applyQuality(image, quality);

                if (format == ImageFormat.TIF || format == ImageFormat.JP2) {
                    writeToBuffer(image, baos, format);
                } else {
                    VTarget vTarget = VTarget.newFromOutputStream(arena, baos);
                    writeToTarget(image, vTarget, format);
                }
            });
        } catch (Exception e) {
